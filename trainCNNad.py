...
class Encoder(nn.Module):
    def __init__(self,word_size,voc_counts,voc_totalCount,word_dim,num_filters,filter_sizes,dropout_p=0.0):
        ...
        self.frec=torch.tensor(voc_counts,dtype=torch.float32,requires_grad=False,device=device)/voc_totalCount
        self.embedding.weight = nn.Parameter(self.embedding.weight.clone().detach().requires_grad_(True))
        ...
        self.frec=self.frec.view(-1,1)
        self.x=None
    def forward(self,input,input_len,train=True, round = 0):
        if train:
          if round == 0:
            Expect = torch.sum(torch.mul(self.embedding.weight, self.frec),0).cuda()
            Var = torch.sum(torch.mul(torch.pow(self.embedding.weight - Expect,2), self.frec))
            self.embedding.weight = nn.Parameter(((self.embedding.weight - Expect)/torch.pow(Var,0.5)).clone().detach().requires_grad_(True))
            x = self.embedding(input)
            self.x=x
            self.x.retain_grad()
          if round == 1:
            x = self.embedding(input)
            x = x + Epsilon*self.x.grad/torch.norm(self.x.grad) #GAN
        else:
          x = self.embedding(input)
        x = x.unsqueeze(1)
...
class SimpleLossCompute:
    ...
    def __call__(self, x, y, norm, train=True, grad=False):
        ...
        if train:
            ...
            loss.backward()
            if grad == True and self.opt is not None:
                self.opt.step()
        else:
